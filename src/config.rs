...
pub enum CustodianSubcommand {
    ...
    Logs(LogsArgs),
    /// Custodian health + version + vault count. Targets the local daemon by
    /// default; set `$SAFECLAW_VAULT_URL` to point at a remote one.
    Status(CommonArgs),
...
pub struct StartArgs {
    /// Overwrite an existing systemd unit even if it wasn't generated by
    /// safeclaw (i.e. lacks the `# Generated by safeclaw` marker). Without
    /// this, `sc custodian start` refuses to clobber a hand-written unit.
    #[arg(long)]
    pub force: bool,
}
