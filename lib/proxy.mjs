...
function sendLockedResponse(res, isStream, unlockUrl) {
  const content = `🔒 SafeClaw is locked. [Tap to unlock](${unlockUrl})`
  const body = {
    id: 'safeclaw-locked',
...
export async function createProxy({ port, dataDir, serverPort }) {
  ...
  const unlockUrl = process.env.SAFECLAW_URL
    ? `${process.env.SAFECLAW_URL}/unlock.html`
    : `http://localhost:${serverPort}/unlock.html`
