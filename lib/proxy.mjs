...
export async function createProxy({ port, dataDir, serverPort }) {
  ...
  const server = http.createServer((req, res) => {
    ...
    if (isLocked()) {
      // Check Accept header and URL for streaming hint before reading body
      const acceptSSE = (req.headers.accept || '').includes('text/event-stream')
      const isStreamUrl = req.url.includes('stream=true')

      const chunks = []
      req.on('data', c => { if (chunks.length < 4) chunks.push(c) }) // cap memory: only read enough to detect stream flag
      req.on('end', () => {
        let isStream = acceptSSE || isStreamUrl
