...
// ── HKDF key derivation ───────────────────────────────────────────────────────
// HKDF-SHA256(ikm=userKey, salt=vmSk_d_bytes, info="safeclaw-kek-v1") → 32-byte KEK
//
// 🔵-1 Documentation: userKey is expected to be 32 bytes, derived client-side from
// WebAuthn PRF output via HKDF normalization:
//   PRF → HKDF(ikm=PRF, salt=zeros(32), info="safeclaw-user-key") → 32-byte userKey
// This normalization ensures consistent key length regardless of PRF output format.
// The HKDF step here (deriveKEK) then uses userKey as IKM and vmSk.d as salt
// to produce a per-VM KEK for wrapping/unwrapping the DEK.

export async function deriveKEK(userKey, vmSk) {
  // userKey is expected to be a 32-byte HKDF-normalized key derived client-side from the
  // WebAuthn PRF output: HKDF(ikm=PRF, salt=zeros, info='safeclaw-user-key') → user_key.
  // This normalization is performed in the browser before the value is sent to the server.
  ...
  const aesKeyBits = await crypto.subtle.deriveBits(
    {
      name: 'HKDF',
      hash: 'SHA-256',
      // Zero salt is acceptable here because the ephemeral key pair provides per-message
