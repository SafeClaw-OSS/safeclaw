...
export async function createServer({ port, dataDir, proxy, hmacSecret, expectedOrigin, rpId, exitFn } = {}) {
  expectedOrigin = expectedOrigin || 'http://localhost'
  rpId = rpId || 'localhost'
  ...
  console.log('[server] HMAC relay verification enabled')
  ...
  const server = http.createServer(async (req, res) => {
    ...
    // HMAC relay verification for POST requests.
    // If hmacSecret is set, all POST requests must include a valid X-Relay-Sig header.
    // If hmacSecret is not set, POST requests from non-localhost origins are rejected.
    if (req.method === 'POST') {
      ...
      if (hmacSecret) {
        ...
        sendJSON(res, 403, { error: 'Remote POST requires HMAC relay signature' })
        return
      }
    }
    ...
    try {
      ...
      // POST /admin/status — requires passkey assertion if vault exists
      if (req.method === 'POST' && pathname === '/admin/status') {
        const passkeysPath = join(dataDir, 'passkeys.json')

        if (!existsSync(passkeysPath)) {
          // No vault yet — nothing sensitive to leak, allow without auth
          sendJSON(res, 200, {
      ...
      if (req.method === 'POST' && pathname === '/setup') {
        ...
        const { passkeys, secrets, userKeys, nonce, assertions, existingCredentialId, existingAssertion } = parsed0
