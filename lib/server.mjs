...
function derToParts(derSig) {
  ...
  if (derSig.length < 8 || derSig[0] !== 0x30) throw new Error('Invalid DER signature: bad header')
  const seqLen = derSig[1]
  if (seqLen + 2 > derSig.length) throw new Error('Invalid DER signature: length mismatch')

  let offset = 2
  // Parse r INTEGER
  if (derSig[offset] !== 0x02) throw new Error('Invalid DER signature: expected INTEGER tag for r')
  const rLen = derSig[offset + 1]
  if (offset + 2 + rLen > derSig.length) throw new Error('Invalid DER signature: r overflows')
  const r = derSig.subarray(offset + 2, offset + 2 + rLen)
  offset += 2 + rLen

  // Parse s INTEGER
  if (derSig[offset] !== 0x02) throw new Error('Invalid DER signature: expected INTEGER tag for s')
  const sLen = derSig[offset + 1]
  if (offset + 2 + sLen > derSig.length) throw new Error('Invalid DER signature: s overflows')
...
export async function createServer({ port, dataDir, proxy, expectedOrigin, rpId, exitFn, rateLimit = 20, onSetup } = {}) {
  ...
  // Append-only nonce log — single-use client nonces
  // Compaction: when file exceeds NONCE_COMPACT_THRESHOLD lines, keep only recent half
  const NONCE_COMPACT_THRESHOLD = 10_000
  ...
  function checkAndRecordNonce(nonceB64) {
    ...
    if (existsSync(noncesPath)) {
      ...
      // Compact if file is too large — keep recent half
      if (lines.length >= NONCE_COMPACT_THRESHOLD) {
        const keep = lines.slice(Math.floor(lines.length / 2))
        writeFileSync(noncesPath, keep.join('\n') + '\n')
      }
    }
  ...
  // ── Auth middleware ──────────────────────────────────────────────────────
  // Decrypts E2E payload, checks nonce, verifies passkey assertion.
  // Returns { parsed, credentialId } or sends error response and returns null.
  async function authenticateRequest(req, res) {
    ...
    return { parsed, credentialId, passkeysData }
  }
  ...
  const server = http.createServer(async (req, res) => {
    ...
    // Per-IP rate limit for all requests (except OPTIONS handled above)
    ...
    try {
      ...
      if (req.method === 'POST' && pathname === '/admin/status') {
        ...
        // Authenticated status — reuse body by injecting into req
        req._body = bodyStr
        ...
        sendJSON(res, 200, {
          ...
          uptime: process.uptime(),
          services: serviceNames,
          passkeys: Object.keys(auth.passkeysData),
        })
        return
      }
      ...
      // POST /unlock
      if (req.method === 'POST' && (pathname === '/admin/unlock' || pathname === '/unlock')) {
        const auth = await authenticateRequest(req, res)
        if (!auth) return

        const { parsed, credentialId } = auth
        ...
        const userKeyBuf = Buffer.from(parsed.userKey, 'base64')
        const kek = await deriveKEK(userKeyBuf, vmSk)
      ...
      if (req.method === 'POST' && pathname === '/admin/credentials') {
        ...
        const userKeyBuf = Buffer.from(parsed.userKey, 'base64')
        const nonceBuf = Buffer.from(parsed.nonce, 'base64')
        const kek = await deriveKEK(userKeyBuf, vmSk)
        const dek = await unwrapDEK(readFileSync(wrappedPath), kek)
        const secretsBuf = await decrypt(dek, readFileSync(join(dataDir, 'vault.enc')))
      ...
      if (req.method === 'POST' && pathname === '/admin/update-secrets') {
        ...
        writeFileSync(join(dataDir, 'vault.enc'), await encrypt(dek, Buffer.from(JSON.stringify(parsed.newSecrets))))
        proxy.setSecrets(parsed.newSecrets)
        serviceNames = Object.keys(parsed.newSecrets.services || {})
      ...
      if (req.method === 'POST' && pathname === '/admin/add-passkey') {
        ...
        const { parsed, credentialId, passkeysData } = auth
        const wrappedPath = join(dataDir, `wrapped_dek_${credentialId}.bin`)
        if (!existsSync(wrappedPath)) {
          sendJSON(res, 401, { error: 'No wrapped DEK for credential' })
          return
        ...
        const { newPasskey, newUserKey: newUserKeyB64 } = parsed
      ...
      if (req.method === 'POST' && pathname === '/admin/remove-passkey') {
        ...
        const { parsed, passkeysData } = auth
        const { removeCredentialId } = parsed
      ...
      const CLEAN_PAGES = { '/setup': 'setup.html', '/unlock': 'unlock.html', '/admin/unlock': 'unlock.html', '/admin': 'admin.html' };
